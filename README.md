...
알고리즘 입문에서 시작해, **매 단계마다 한계에 부딪히고 그걸 풀려고 다음 도구로 옮겨간** 궤적입니다.
예측 → 의사결정 → 실행, Python → Julia → Rust.

**2022 · 코딩의 시작**
트레이딩 아이디어는 있는데 구현할 도구가 없었다 — 알고리즘·자료구조·Python 기초부터.
`baekjoon-algorithm` · `Data-structure` · `practice_openCV`

**2022–2023 · AI · 데이터 분석**
- 🔴 **문제** — 시장 데이터에서 어떻게 *의미 있는 신호*를 뽑아낼까?
- 🟢 **해결** — ML·통계로 팩터/시그널 분석, 감성분석·EDA. 금융 데이터에 머신러닝을 적용하기 시작.
- `machine-learning` · `ML for Factor Investing` · `mini-project`

**2023–2024 · 강화학습**
- 🔴 **문제** — 지도학습은 *단발 예측*일 뿐, 사고·팔고·보유의 순차적 의사결정을 담지 못한다.
- 🟢 **해결** — 트레이딩을 *순차 의사결정 문제*로 재정의하고 RL로 전환. 커스텀 차트 환경·스캘핑 에이전트, 연구실에서 조합최적화 RL(TSP·VRP).
...
- `lob-deep-learning` · `deepOBs` · `online_estimation` · `ML-HFT`

**2025–2026 · Julia 알파 리서치**
...
**2026 · Julia + Rust 하이브리드 OMS**
- 🔴 **문제** — 리서치를 실거래로 옮기려니 *저지연 주문 실행 인프라*가 필요. Julia만으론 핫패스 성능 부족.
- 🟢 **해결** — 성능 핫패스를 Rust DLL로 빼낸 Julia + Rust 하이브리드 OMS 구축.
- `Julia + Rust DLL (ccall FFI)`

**2026 · 현재 — 순수 Rust OMS** 🦀
- 🔴 **문제** — FFI 경계 오버헤드 + Julia *GC 압박*이 레이턴시·안정성의 병목.
